...
jack_midi_data_t controlChange56[3] = {0x30, 0x38, 0x00};


int process(jack_nframes_t nframes, void *arg)
{
	// You must get the buffer each cycle.
	void *inputBuffer = jack_port_get_buffer(inputPort, nframes);

	// You must get the buffer each cycle.
	void *outputBuffer = jack_port_get_buffer(outputPort, nframes);
	...
	if (inputEventCount > 0)
	{
		...
		controlChange56[0] = 0x30;
		controlChange56[1] = 0x38;
		controlChange56[2] = controlValue;

		// Send the data out to the device.
		printf("Midi Write: [%0X]\n", controlValue);
		jack_midi_event_write(outputBuffer, nframes, controlChange56, 3);
		...
		for (jack_nframes_t i = 0; i < inputEventCount; i++)
		{
			...
			if (success == 0)
			{
				...
				switch (messageType)
				{
					case 0:
						printf(
						    "Note Off:        Chan: [%0X]    Data: [%0X][%0X]\n", channel, event->buffer[1],
						    event->buffer[2]);
						break;

					case 1:
						printf(
						    "Note On:         Chan: [%0X]    Data: [%0X][%0X]\n", channel, event->buffer[1],
						    event->buffer[2]);
						break;

					case 2:
						printf(
						    "P. Aftertouch:   Chan: [%0X]    Data: [%0X][%0X]\n", channel, event->buffer[1],
						    event->buffer[2]);
						break;

					case 3:
						printf(
						    "Control Change:  Chan: [%0X]    Data: [%0X][%0X]\n", channel, event->buffer[1],
						    event->buffer[2]);
						break;

					case 4:
						printf(
						    "Program Change:  Chan: [%0X]    Data: [%0X][%0X]\n", channel, event->buffer[1],
						    event->buffer[2]);
						break;

					case 5:
						printf(
						    "C. Aftertouch:   Chan: [%0X]    Data: [%0X][%0X]\n", channel, event->buffer[1],
						    event->buffer[2]);
						break;

					case 6:
						printf(
						    "Pitch Wheel:     Chan: [%0X]    Data: [%0X][%0X]\n", channel, event->buffer[1],
						    event->buffer[2]);
						break;
				}
			}
